...
description: |
  ...
  采用懒加载引导式初始化，无需前置配置即可使用。配额管理复用 global-biblio-base 的 gateway 凭证和计次规则（v3.9.0：5接口，每次调用计1次），配额耗尽后暂停外部检索请求。
...
- **版本**：v3.9.0
- **核心理念**：基于 SmartLib 开放平台 API 提供中外文学术文献检索，已接入全程自动化服务（gateway 注册 + 配额消耗 + 统一钱包计费与升级）
...
本技能与 global-biblio-base 共享**统一钱包**：套餐价格、额度与升级流程见 global-biblio-base 的 `references/account.md`（smartlib-account）；配额不足或想升级时说「我要升级」即可。
...
📥 下载额度：剩余 {download_remain} 次（含免费 {download_quota_free} 次 + 付费 {download_paid_remain} 次；每月 {download_reset_at} 重置）
```

> `/quota` 完整返回含下载字段：`download_quota_free`（免费月度额度）、`download_reset_at`（重置日）、`download_paid_remain`（付费授权剩余）、`download_remain`（合计可用）。
...
| **global-biblio-base** (v3.9.0) | 检索结果一键入库；SmartLib 补全字段；共享配额与凭证；外部检索联动 | ⬆️ 上游参考 |
...
| v3.11.3 | 2026-07-19 | 文档对齐 v52.1 统一钱包：上游 global-biblio-base 版本标注更新为 v3.9.0；/quota 输出规范补充下载额度展示（download_quota_free / download_reset_at / download_paid_remain / download_remain） |
